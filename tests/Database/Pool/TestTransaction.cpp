 ...
 *
 * @details 用真实 SQLite 钉住事务的三条核心语义：
 *          - 提交前别的连接看不到本次写入，提交后才可见（证明写语句确实在事务里）；
 *          - 回滚后写入不可见（包括析构自动回滚与异常穿越作用域两条路径）；
 *          - 重复 commit / rollback 幂等，且不会误撤销已提交的工作。
 *          另外覆盖「事务持有连接」这一前提（池被占满时第二个事务拿不到连接，
 *          而不是共享第一个事务的连接）与批量插入的分块执行。
 *
 * 为什么用文件库而不是 ":memory:"：内存库不跨连接共享，只有一个连接时根本观察不到
 * 「提交前不可见」，事务语义就无从验证。文件库配合默认启用的 WAL 模式，
 * 读连接不会被写事务阻塞。
 *
 * 覆盖场景：
