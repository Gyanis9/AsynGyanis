/**
 * @file TestHttpMemoryBudget.cpp
 * @brief 在途正文全局预算的用例：额度语义、并发下不超发，以及超预算请求被 503 收口
 * @details 预算类本身是纯原子的账本，前四个用例直接测它；最后一个用例跑真实服务器，验证
 *          「正文边收边攒时就判定」这条时机：声明 100 字节正文、只发 64 字节，此刻若预算不够
 *          会话就该收口，而不是等收齐（收齐时内存已经占住了）。
 * @author Gyanis
 * @date 2026-09-13
 * @version 1.0.0
