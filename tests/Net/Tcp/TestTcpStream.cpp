...
namespace AsynGyanis::Net
{
    namespace
    {
         ...
         */
        bool runAndAwaitCompletion(EventLoopThread &loopThread, Core::Task<> &driverTask, OperationOutcome &outcome,
         ...
         */
        bool readFromPeerUntil(const int descriptor, std::string &received, const std::function<bool(const std::string &)> &predicate,
                               const std::chrono::milliseconds timeout)
        {
            const auto deadline = std::chrono::steady_clock::now() + timeout;
    ...
    TEST(TcpStream, FirstReadReturnsOnlyPayloadSentByPeer)
    {
        ...
        Core::Task<> driverTask = runOnEventLoop(outcome, [&stream, &outcome, &receiveStorage]() -> Core::Task<>
        {
    ...
    TEST(TcpStream, ReadClampsToInternalBufferCapacityAndKeepsRemainder)
    {
        ...
        Core::Task<> driverTask = runOnEventLoop(firstOutcome, [&stream, &firstOutcome, &secondOutcome, &firstStorage, &secondStorage]() -> Core::Task<>
        {
        ...
        ASSERT_TRUE(waitForCondition(
                [&secondOutcome]
                {
                    return secondOutcome.completed.load(std::memory_order_acquire);
                },
                kLargePayloadWaitTimeout)) << "续读超时：等待上界为 kLargePayloadWaitTimeout，属依赖时序的一步";
    ...
    TEST(TcpStream, ReadConsumesBufferedBytesInOrderBeforeReadingAgain)
    {
        ...
        TcpStream stream(Core::AsyncSocket(loop, descriptors.takeStreamSide()));
        OperationOutcome outcome;
        Core::Task<> driverTask = runOnEventLoop(outcome, [&stream, &outcome]() -> Core::Task<>
        {
    ...
    TEST(TcpStream, ReadUntilReturnsTextBeforeDelimiterAndSkipsIt)
    {
        ...
        OperationOutcome firstOutcome;
        OperationOutcome secondOutcome;
        Core::Task<> driverTask = runOnEventLoop(firstOutcome, [&stream, &firstOutcome, &secondOutcome]() -> Core::Task<>
        {
    ...
    TEST(TcpStream, ReadUntilTruncationAdvancesConsumedPosition)
    {
        ...
        Core::Task<> driverTask = runOnEventLoop(firstOutcome, [&stream, &firstOutcome, &secondOutcome, &thirdOutcome]() -> Core::Task<>
        {
        ...
        ASSERT_TRUE(waitForCondition(
                [&thirdOutcome]
                {
                    return thirdOutcome.completed.load(std::memory_order_acquire);
                },
                kWaitTimeout));
    ...
    TEST(TcpStream, WriteZeroLengthDoesNotSendAnything)
    {
        ...
        std::string received;
        EXPECT_FALSE(readFromPeerUntil(descriptors.peerSide(), received,
    ...
    TEST(TcpStream, WriteAllSendsWholePayloadToPeer)
    {
        ...
        TcpStream stream(Core::AsyncSocket(loop, descriptors.takeStreamSide()));
        OperationOutcome outcome;
        Core::Task<> driverTask = runOnEventLoop(outcome, [&stream, &outcome, &payload]() -> Core::Task<>
        {
        ...
        std::string received;
        ASSERT_TRUE(readFromPeerUntil(descriptors.peerSide(), received,
                                      [&](const std::string &accumulated)
                                      {
                                          return accumulated.size() >= payload.size();
                                      },
                                      kLargePayloadWaitTimeout));
    ...
    TEST(TcpStream, MoveConstructionTransfersSocketAndBufferedBytes)
    {
        ...
        OperationOutcome outcome;
        Core::Task<> driverTask = runOnEventLoop(outcome, [&sourceStream, &outcome]() -> Core::Task<>
        {
