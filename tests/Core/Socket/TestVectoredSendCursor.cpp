/**
 * @file TestVectoredSendCursor.cpp
 * @brief 聚合发送游标单元测试：部分写之后的段内偏移、跨段推进与完成判定
 * @author Gyanis
 * @date 2026-09-12
 * @version 1.0.0
 * @copyright Copyright (c) . All rights reserved.
 *
 * @details 这段推进逻辑在回环上很难被自然触发（Windows 上一次 WSASend 能吞下远超接收窗口
 *          的数据，实测 1 MiB 也是一次写完），但它一旦写错就是**静默的数据错位**：
 *          线上字节流多一段、少一段或顺序乱了，都不会有任何错误返回。因此把账目抽成游标，
 *          在这里用确定性用例逐条钉住——首段偏移、跨段、一次跨过若干段、零长度段与完成判定。
 */
