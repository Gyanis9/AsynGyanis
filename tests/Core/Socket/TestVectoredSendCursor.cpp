 ...
 *
 * @details 这段推进逻辑在回环上很难被自然触发（Windows 上一次 WSASend 能吞下远超接收窗口的数据），
 *          但它一旦写错就是**静默的数据错位**：线上字节流多一段、少一段或顺序乱了都不会有任何错误返回。
 *          因此把账目抽成游标，在这里用确定性用例逐条钉住——首段偏移、跨段、一次跨过若干段、零长度段与完成判定。
 */
