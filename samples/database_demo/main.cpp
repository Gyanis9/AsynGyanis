...
namespace
{
     ...
     */
    constexpr std::string_view kCreateRawTableSql = "CREATE TABLE raw_values ("
...
namespace
{
    ...
    public:
        ...
        explicit TemporaryDatabaseFile(const std::string_view purpose) :
            m_path(std::filesystem::temp_directory_path() /
                   ("asyn-sample-database-" + std::string(purpose) + "-" + std::to_string(Platform::ProcessInfo::currentProcessId()) + ".db"))
        {
    ...
    {
        const std::string portText   = Platform::ProcessInfo::environmentVariable(std::string{variableName}).value_or(std::string{});
        int               parsedPort = 0;
        const auto        result     = std::from_chars(portText.data(), portText.data() + portText.size(), parsedPort);
        if (portText.empty() || result.ec != std::errc{} || result.ptr != portText.data() + portText.size() || parsedPort <= 0 || parsedPort > 65535)
        {
            return fallback;
        }
    ...
    {
        const std::string valueText   = Platform::ProcessInfo::environmentVariable(std::string{variableName}).value_or(std::string{});
        int               parsedValue = 0;
    ...
    std::function<std::unique_ptr<Database::DatabaseConnection>()> makeSqliteFactory(const std::string &databasePath)
    {
        ...
        {
            auto connection = std::make_unique<Database::SqliteConnection>(Database::ConnectionConfig::sqliteDefault(databasePath));
     ...
     * @param body 步骤体
     */
    void runGatedStep(const std::string_view stepName, const bool isAvailable, const std::string_view skipReason, const std::function<void()> &body)
    {
        if (!isAvailable)
        {
    ...
    void demonstrateSqliteFileDatabase(const TemporaryDatabaseFile &databaseFile)
    {
        ...
        Samples::checklist().check(scalarInteger(connection, "PRAGMA busy_timeout") == std::optional<std::int64_t>(4321), "queryTimeout() 真的映射成了 PRAGMA busy_timeout");
        ...
        const std::unique_ptr<Database::DatabaseResult> overLimitSelection =
                connection.execute("WITH RECURSIVE tick (n) AS (SELECT 1 UNION ALL SELECT n + 1 FROM tick WHERE n < 200000000)"
        ...
        connection.setQueryTimeout(4321);
        Samples::checklist().check(scalarInteger(connection, "SELECT 1") == std::optional<std::int64_t>(1), "被打断之后同一条连接照常能查询");
        ...
        if (const std::unique_ptr<Database::DatabaseResult> selection = connection.execute("SELECT id, label, ratio, age, payload FROM raw_values ORDER BY id"))
        {
            ...
            for (std::size_t rowIndex = 0; result.next(); ++rowIndex)
            {
                ...
                if (rowIndex == 0)
                {
                    ...
                    if (const std::optional<Database::BinaryBytes> bytes = cellAs<Database::BinaryBytes>(result, std::string_view{"payload"}); bytes.has_value())
                    {
                        isBlobKeepsEmbeddedNull = bytes->size() == 3 && (*bytes)[0] == 0x5C && (*bytes)[1] == 0x00 && (*bytes)[2] == 0x41;
                    }
                } else
                {
            ...
            isCursorCorrect = ids == std::vector<std::int64_t>{1, 2} && firstLabel == std::optional<std::string>("张三") && firstRatio == std::optional<double>(95.5) &&
                              secondRatio == std::optional<double>(-0.25) && isAgeMissingOnSecondRow && isBlobKeepsEmbeddedNull && isRewound && result.rowCount() == 2 &&
        ...
        Samples::checklist().check(isCursorCorrect, "结果集游标逐列读回：整数/文本/浮点/NULL 各归其类，内嵌 '\\0' 的二进制按长度保真，reset() 能重新遍历");

        // 写回执上的两个计数：影响行数走基类虚接口即可读到，不必向下转型
        bool isReceiptCorrect = false;
        if (const std::unique_ptr<Database::DatabaseResult> receipt = connection.execute("INSERT INTO raw_values (id, label) VALUES (500, 'Explicit')"))
        {
        ...
        Samples::checklist().check(isReceiptCorrect, "写回执：affectedRowCount() 经基类接口读到 1，rowid 计数器被显式主键刷新到 500");
        ...
        Samples::checklist().check(isReaderConnected && scalarInteger(reader, "SELECT COUNT(*) FROM raw_values") == std::optional<std::int64_t>(3) &&
                                           scalarText(reader, "PRAGMA journal_mode") == std::optional<std::string>("wal"),
                                   "第二个连接对象读到已提交的三行，且文件库确实跑在 WAL 模式下");
        Samples::checklist().check(reader.serverVersion().find('.') != std::string::npos && !reader.serverVersion().empty(), "serverVersion() 给出链接进来的 SQLite 版本号");
    }
    ...
    void demonstrateSqliteInMemoryDatabase()
    {
        ...
        const bool isSeeded =
                connection.execute(kCreateRawTableSql) != nullptr && connection.execute("INSERT INTO raw_values (id, label, ratio, age) VALUES (1, '甲', 0.5, 7)") != nullptr;
        Samples::checklist().check(isSeeded && scalarInteger(connection, "SELECT COUNT(*) FROM raw_values") == std::optional<std::int64_t>(1) &&
        ...
        Samples::checklist().check(fallbackConnection.connect() && scalarText(fallbackConnection, "PRAGMA journal_mode") == std::optional<std::string>("memory"),
                                   "database 为空串时兜底成内存库并立即可用");

        // 内存库随句柄销毁一起清空：重连拿到的是全新空库，失败原因里既有中文也有引擎原文
        connection.disconnect();
        Samples::checklist().check(connection.connect() && connection.execute("SELECT * FROM raw_values") == nullptr && containsChinese(connection.lastError()) &&
    ...
    void demonstrateFactoryAndTypes(const TemporaryDatabaseFile &databaseFile)
    {
        ...
        std::unique_ptr<Database::DatabaseConnection> connection = Database::DatabaseFactory::createSqlite(Database::ConnectionConfig::sqliteDefault(databaseFile.utf8Path()));
        Samples::checklist().check(connection != nullptr && connection->databaseType() == Database::DatabaseType::Sqlite &&
                                           std::string_view(Database::databaseTypeName(Database::DatabaseType::Sqlite)) == "Sqlite" && !connection->isConnected() &&
                                           connection->connect() && databaseFile.exists() && connection->configuration().database == databaseFile.utf8Path(),
        ...
        Samples::checklist().check(scalarInteger(*connection, "SELECT COUNT(*) FROM raw_values") == std::optional<std::int64_t>(1) && selection != nullptr && selection->next() &&
                                           cellAs<std::string>(*selection, std::size_t{0}) == std::optional<std::string>("工厂写入"),
                                   "经基类指针读写文件库：虚派发把 SQL 真的送到了 SQLite 驱动");

        Samples::checklist().check(Database::DatabaseFactory::guessType(3306) == std::optional<Database::DatabaseType>(Database::DatabaseType::MySql) &&
        ...
        const std::filesystem::path missingDirectoryPath =
                std::filesystem::temp_directory_path() / "asyn-sample-database-missing-directory" / std::to_string(Platform::ProcessInfo::currentProcessId()) / "database.db";
        ...
        Samples::checklist().check(!brokenConnection.connect() && !brokenConnection.isConnected() && brokenConnection.nativeHandle() == nullptr &&
    ...
    void demonstrateDialectLayer()
    {
        ...
        Samples::checklist().check(statement.sql.find("WHERE") != std::string::npos && !statement.sql.empty() &&
        ...
        Samples::checklist().check(statement.sql.find("\"sample accounts\"") != std::string::npos && statement.sql.find("\"name\"") != std::string::npos &&
                                           statement.sql.find("LIMIT 5") != std::string::npos,
                                   "含空格的表名与列名在翻译后的 SQL 里被整段引用，分页按方言补全");

        Samples::checklist().check(sqliteDialect.beginTransactionStatement() == "BEGIN IMMEDIATE" && mySqlDialect.beginTransactionStatement() == "START TRANSACTION" &&
                                           sqliteDialect.commitStatement() == "COMMIT" && sqliteDialect.rollbackStatement() == "ROLLBACK",
                                   "事务控制语句的文本由方言给出，SQLite 与 MySQL 的写法在此处分家");

        // 两个引擎的能力差异：参数上限与「能不能直接做主键」的物理类型
        Samples::checklist().check(sqliteDialect.maximumStatementParameters() == 999 && mySqlDialect.maximumStatementParameters() > 999 &&
        ...
        Samples::checklist().check(Database::DialectRegistry::supports(Database::DatabaseType::Sqlite) && Database::DialectRegistry::supports(Database::DatabaseType::MySql) &&
    ...
    {
        return AccountRow{.id = 2, .name = "O'Brien -- DROP TABLE sample accounts; --", .balance = -99.5, .note = std::nullopt, .active = false};
    }
    ...
    void demonstrateObjectRelationalMapping(Database::ConnectionPool &pool)
    {
        ...
        const Database::SqlStatement createStatement =
                Database::Queryable::SchemaMigrator::createTableStatement<AccountRow>(*Database::DialectRegistry::dialectFor(Database::DatabaseType::Sqlite));
        Samples::checklist().check(createStatement.sql.find("CREATE TABLE IF NOT EXISTS \"sample accounts\"") == 0 && createStatement.parameters.empty() &&
        ...
        Samples::checklist().check(isCreated && errorText.empty() && Database::Queryable::SchemaMigrator::tableExists<AccountRow>(pool, &errorText) && errorText.empty() &&
                                           !Database::Queryable::SchemaMigrator::tableExists<DocumentRow>(pool, &errorText) && errorText.empty(),
                                   "SchemaMigrator 建表成功，tableExists 能区分「已建的表」与「还没建的表」");

        OrmQuery<AccountRow> insertQuery(pool, Database::DatabaseType::Sqlite);
        const std::int64_t   firstInserted  = insertQuery.insert(AccountRow{.id = 1, .name = "张三", .balance = 1234.56, .note = std::string("普通备注"), .active = true});
        const std::int64_t   secondInserted = insertQuery.insert(makeHostileSampleRow());
        const std::int64_t   thirdInserted  = insertQuery.insert(AccountRow{.id = 3, .name = "李四", .balance = 0.0, .note = std::string("中文备注"), .active = true});

        OrmQuery<AccountRow> countQuery(pool, Database::DatabaseType::Sqlite);
        Samples::checklist().check(firstInserted == 1 && secondInserted == 1 && thirdInserted == 1 && countQuery.count() == 3, "ORM 插入三行各回报 1 行受影响，count() 数到三行");
        ...
        Samples::checklist().check(rows.size() == 3 && rows[0].name == "张三" && rows[0].balance == 1234.56 && rows[0].note == std::optional<std::string>("普通备注") &&
                                           rows[0].active && rows[1].id == 2 && rows[1].name == makeHostileSampleRow().name && !rows[1].note.has_value() &&
                                           rows[1].balance == -99.5 && !rows[1].active && rows[2].name == "李四" && rows[2].balance == 0.0 &&
        ...
        const std::vector<AccountRow> topRows = orderedQuery.where(kAccountIdColumn >= std::int64_t{1}).orderBy(Database::Queryable::desc("id")).limit(1).toList();
        ...
        const std::vector<AccountRow> matched =
                filteredQuery
                        .where(Database::Queryable::in(kAccountIdColumn, std::vector<std::int64_t>{1, 3}) && Database::Queryable::like(kAccountNameColumn, std::string("%张%")))
                        .toList();
        ...
        const std::size_t    wildcardRows = wildcardQuery.where(Database::Queryable::like(kAccountNameColumn, std::string("%"))).toList().size();
        const std::size_t    literalRows  = literalQuery.where(Database::Queryable::contains(kAccountNameColumn, std::string("%"))).toList().size();
        Samples::checklist().check(wildcardRows == 3 && literalRows < wildcardRows, "同一个 '%'：like 当通配符命中全部三行，contains 当字面量只匹配真含百分号的行");
        ...
        Samples::checklist().check(prefixRows.size() == 1 && prefixRows[0].id == 1 && suffixRows == 1, "startsWith / endsWith 按字面量匹配前缀与后缀，各只命中一行");

        OrmQuery<AccountRow> missingQuery(pool, Database::DatabaseType::Sqlite);
        Samples::checklist().check(!missingQuery.where(kAccountIdColumn == std::int64_t{999}).first().has_value(), "first() 在无匹配行时给出空 optional 而不是抛异常");
        ...
        Samples::checklist().check(activeCountQuery.where(kAccountActiveColumn == true).count() == 2 && negativeCountQuery.where(kAccountBalanceColumn < 0.0).count() == 1,
                                   "带条件的 count() 按谓词过滤：启用两行、负余额一行");

        OrmQuery<AccountRow>          updateQuery(pool, Database::DatabaseType::Sqlite);
        const std::int64_t            updatedRows = updateQuery.update(AccountRow{.id = 2, .name = "王五", .balance = 888.25, .note = std::string("已更新"), .active = true});
        ...
        Samples::checklist().check(updatedRows == 1 && afterUpdate.size() == 3 && afterUpdate[1].name == "王五" && afterUpdate[1].balance == 888.25 &&
                                           afterUpdate[1].note == std::optional<std::string>("已更新") && afterUpdate[0].name == "张三" && afterUpdate[2].name == "李四",
        ...
        {
            batchRows.push_back(AccountRow{.id = id, .name = "批量-" + std::to_string(id), .balance = static_cast<double>(id), .note = std::nullopt, .active = false});
        }
        ...
        Samples::checklist().check(batchInsertedRows == 300 && totalRowCount == 303 && secondChunkRow.has_value() && secondChunkRow->name == "批量-250" &&
                                           secondChunkRow->balance == 250.0 && !secondChunkRow->note.has_value(),
        ...
        Samples::checklist().check(clearQuery.executeNonQuery() == 3 && clearedCountQuery.count() == 0, "不带条件的 executeNonQuery 清空全表（文档承诺的语义，写侧同样走方言）");
        ...
        Samples::checklist().check(offlineQuery.toSql() == "SELECT * FROM sample accounts" && isOfflineExecutionRejected && containsChinese(offlineReason),
    ...
    void demonstrateTransactions(Database::ConnectionPool &pool)
    {
...

        {
            ...
            const std::int64_t    firstInserted =
                    transactionalQuery.insert(AccountRow{.id = 1, .name = "提交行", .balance = 1234.56, .note = std::string("普通备注"), .active = true});
            const std::int64_t secondInserted = transactionalQuery.insert(AccountRow{.id = 2, .name = "李四", .balance = -99.5, .note = std::nullopt, .active = false});

            const std::int64_t invisibleRowCount = bystanderCount();
            const bool         isCommitted       = transaction.commit();

            Samples::checklist().check(transaction.isActive() == false && firstInserted == 1 && secondInserted == 1 && invisibleRowCount == 0 && isCommitted,
                                       "提交前旁观连接一行都看不到，commit() 才让它生效");

            OrmQuery<AccountRow>            readBack(pool, Database::DatabaseType::Sqlite);
            const std::optional<AccountRow> committedRow = readBack.where(kAccountIdColumn == std::int64_t{1}).first();
            Samples::checklist().check(bystanderCount() == 2 && committedRow.has_value() && committedRow->name == "提交行" && committedRow->balance == 1234.56 &&
                                               committedRow->note == std::optional<std::string>("普通备注"),
                                       "提交后数据对别的连接可见，字段逐列回读一致");

            // 已结束的事务再喊提交/回滚都不该发出多余语句，更不该撤掉已落库的数据
            Samples::checklist().check(transaction.commit() && transaction.rollback() && bystanderCount() == 2, "commit 之后再 commit / rollback 都幂等，已提交的数据不受影响");
        }

        {
            Database::Transaction transaction(pool);
            OrmQuery<AccountRow>  transactionalQuery(transaction);
            static_cast<void>(transactionalQuery.insert(AccountRow{.id = 3, .name = "会被回滚", .balance = 1.0, .note = std::nullopt, .active = true}));
            static_cast<void>(transactionalQuery.insert(AccountRow{.id = 4, .name = "也会被回滚", .balance = 2.0, .note = std::nullopt, .active = true}));
            const bool isRolledBack = transaction.rollback();

            Samples::checklist().check(isRolledBack && !transaction.isActive() && bystanderCount() == 2, "显式回滚撤销本次两行写入，表回到事务开始前的样子");
        }

        {
            Database::Transaction transaction(pool);
            OrmQuery<AccountRow>  transactionalQuery(transaction);
            static_cast<void>(transactionalQuery.insert(AccountRow{.id = 5, .name = "未提交就离开作用域", .balance = 3.0, .note = std::nullopt, .active = true}));
            // 刻意不调用 commit / rollback：析构必须补上回滚，否则未结束的事务会串给下一个借用者
        }
        Samples::checklist().check(bystanderCount() == 2, "未提交就离开作用域不会留下半成品（事务析构与归还时的会话复位都会兜底，"
        ...
        try
        {
            Database::Transaction transaction(pool);
            OrmQuery<AccountRow>  transactionalQuery(transaction);
            static_cast<void>(transactionalQuery.insert(AccountRow{.id = 6, .name = "回滚掉的新行", .balance = 7.0, .note = std::nullopt, .active = true}));
            // 主键 1 已提交：这一条必败，并把整笔事务带进异常路径
            static_cast<void>(transactionalQuery.insert(AccountRow{.id = 1, .name = "重复主键", .balance = 8.0, .note = std::nullopt, .active = true}));
        } catch (const std::exception &caught)
        {
        ...
        static_cast<void>(connection.connect());
        static_cast<void>(connection.execute(kCreateRawTableSql));
        const bool isNestedBeginRejected   = connection.beginTransaction() && connection.execute("INSERT INTO raw_values (id, label) VALUES (1, 'InsideTx')") != nullptr &&
        ...
        Samples::checklist().check(!connection.commit() && !connection.rollback() && containsChinese(connection.lastError()) && connection.execute("SELECT 1") != nullptr,
    ...
    void demonstrateBinaryColumn(Database::ConnectionPool &pool)
    {
        ...
        Samples::checklist().check(payloadStorageClass == std::optional<std::string>("blob") && rawStorageClass == std::optional<std::string>("blob"),
        ...
        Samples::checklist().check(rows.size() == 2 && rows[0].note.has_value() && rows[0].note->empty() && !rows[1].note.has_value() && rows[1].payload.empty(),
    ...
    void demonstrateParameterBindingAndRejections(Database::ConnectionPool &pool)
    {
        ...
        const std::vector<Database::DatabaseValue>      scalarParameters{std::int64_t{7}, std::string("O'Brien -- 中文"), -12.5, true, Database::BinaryBytes{0x00, 0xFF, 0x10}};
        ...
        if (isRoundTripCorrect)
        {
            ...
            if (const std::unique_ptr<Database::DatabaseResult> selection =
                        connection.execute("SELECT label, ratio, age, payload, typeof(payload) AS payloadType FROM raw_values WHERE id = ?", selectParameters))
            {
                ...
                if (isRoundTripCorrect)
                {
                    ...
                    isRoundTripCorrect = cellAs<std::string>(*selection, std::string_view{"label"}) == std::optional<std::string>("O'Brien -- 中文") &&
                                         ...
                                         cellAs<std::string>(*selection, std::string_view{"payloadType"}) == std::optional<std::string>("blob") && payloadBytes.has_value() &&
                                         payloadBytes->size() == 3 && (*payloadBytes)[0] == 0x00 && (*payloadBytes)[1] == 0xFF;
                }
            } else
            {
        ...
        Samples::checklist().check(isRoundTripCorrect, "五类标量与二进制参数按位置绑定：含单引号与 \"--\" 的文本原样存回，二进制仍落在 blob 存储类");
        ...
        const bool isNullAndEmptyDistinct = connection.execute("INSERT INTO raw_values (id, label, ratio) VALUES (?, ?, ?)", nullParameters) != nullptr &&
        ...
        const bool isManyRejected = connection.execute("SELECT id FROM raw_values", std::vector<Database::DatabaseValue>{std::int64_t{1}}) == nullptr &&
                                    connection.lastError().find("参数数量不匹配") != std::string::npos;
        ...
        const bool isContainerRejected = connection.execute("SELECT ?", std::vector<Database::DatabaseValue>{std::vector<std::string>{"甲", "乙"}}) == nullptr &&
        ...
        Samples::checklist().check(brokenResult == nullptr && containsChinese(connection.lastError()) && connection.lastError().find("no such table") != std::string::npos &&
                                           connection.lastError().find("错误码") != std::string::npos,
                                   "坏 SQL 以 nullptr + 「中文动作说明 + 引擎原文 + 错误码」暴露，不抛不崩");

        // 一次调用只接受一条语句：半执行比整次失败更难排查，第一条建表也不许留下
        const std::unique_ptr<Database::DatabaseResult> multiStatementResult = connection.execute("CREATE TABLE pairs (leftValue INTEGER); INSERT INTO pairs VALUES (1, 2);");
        const std::string                               multiStatementError  = connection.lastError();
        const bool isMultiStatementRejected = multiStatementResult == nullptr && containsChinese(multiStatementError) && connection.execute("SELECT * FROM pairs") == nullptr;
        ...
        try
        {
            ...
            static_cast<void>(duplicating.insert(AccountRow{.id = 700, .name = "第一次", .balance = 1.0, .note = std::nullopt, .active = true}));
            OrmQuery<AccountRow> conflicting(pool, Database::DatabaseType::Sqlite);
            static_cast<void>(conflicting.insert(AccountRow{.id = 700, .name = "重复主键", .balance = 2.0, .note = std::nullopt, .active = true}));
        ...
        Samples::checklist().check(isExceptionAsExpected && afterQuery.count() == countBefore + 1, "重复主键经 ORM 抛出 QueryExecutionException（中文原因），失败的那一行没有落库");
        ...
        Samples::checklist().check(firstGeneratedId > 0 && secondGeneratedId == firstGeneratedId + 1, "写回执带着本条语句生成的自增标识，且各自快照不被后来的插入推走");
    }
    ...
    void demonstrateConnectionPool(Database::ConnectionPool &pool)
    {
        ...
        Samples::checklist().check(static_cast<bool>(first) && static_cast<bool>(second) && !static_cast<bool>(third) && pool.activeCount() == 2 && pool.idleCount() == 0 &&
                                           pool.waitingCount() == 0 && pool.borrowTimeoutCount() == 0,
        ...
        Samples::checklist().check(isMoved && static_cast<bool>(moved) && !static_cast<bool>(second), "PooledConnection 的移动语义：目标持有连接，源对象变空不会重复归还");
        moved.release();
        Samples::checklist().check(!static_cast<bool>(moved) && pool.activeCount() == 1 && pool.idleCount() == 1, "release() 提前归还后包装器为空，活跃数换到空闲数");

        // LIFO 复用：刚归还的那条连接应当是下一次拿到的同一条，而不是每次都新建
        Database::PooledConnection reBorrowed = pool.acquire();
        Samples::checklist().check(static_cast<bool>(reBorrowed) && reBorrowed.operator->() == returnedConnection && reBorrowed.operator->() != first.operator->(),
        ...
        const ScopedCleanup joinWaiterThread(
                [&waiter]
                {
                    if (waiter.joinable())
                    {
        ...
        const bool isWaiterQueued = Samples::waitUntil([&resetPool] { return resetPool.waitingCount() > 0; }, std::chrono::seconds{5}, std::chrono::milliseconds{5});
        held.release();
        const bool isServedInTime =
                Samples::waitUntil([&isWaiterServed] { return isWaiterServed.load(std::memory_order_acquire); }, std::chrono::seconds{10}, std::chrono::milliseconds{5});
        waiter.join();
        Samples::checklist().check(isWaiterQueued && isServedInTime && isWaiterHoldsConnection.load(), "归还一条连接让排队的下一个借用者进得来（不是等自己的获取超时）");
        ...
        Samples::checklist().check(!static_cast<bool>(exhaustedPool.acquire()) && exhaustedPool.totalCount() == 0 && exhaustedPool.borrowTimeoutCount() == 1 &&
                                           exhaustedPool.createdCount() == 0,
    ...
    Core::Task<> runAsyncProbe(Database::ConnectionPool &pool, Core::AsyncExecutor &executor, Core::EventLoop &loop, AsyncProbeResult &outcome)
    {
        try
        {
            OrmQuery<AccountRow> insertQuery(pool, Database::DatabaseType::Sqlite);
            insertQuery.useAsyncExecutor(executor);
            outcome.insertedRows = co_await insertQuery.insertAsync(AccountRow{.id = 21, .name = "异步写入", .balance = 1.5, .note = std::string("异步"), .active = true}, loop);
            ...
            outcome.isPoolConnectionUsable = static_cast<bool>(borrowed) && scalarInteger(*borrowed, "SELECT COUNT(*) FROM \"sample accounts\"") == std::optional<std::int64_t>(1);
    ...
    void demonstrateAsyncSurface(const TemporaryDatabaseFile &databaseFile)
    {
        ...
        const bool            isLoopRunning = Samples::waitUntil([&loop] { return loop.isRunning(); }, std::chrono::seconds{5}, std::chrono::milliseconds{5});
        ...
        const bool isProbeFinished = isLoopRunning && Samples::waitUntil([&outcome] { return outcome.isFinished.load(std::memory_order_acquire); }, std::chrono::seconds{20},
                                                                         std::chrono::milliseconds{5});

        loop.stop();
        loopThread.join();

        Samples::checklist().check(isProbeFinished && outcome.errorText.empty(), "异步链路的探针在时限内跑完且协程里没有异常（没有任何一次 await 挂死）");
        ...
        LOG_INFO_FMT("异步探针的恢复线程与事件循环线程{}一致", outcome.resumedThreadId == loopThreadId ? "" : "不");
        ...
        Samples::checklist().check(written.has_value() && written->name == "异步写入" && written->balance == 1.5 && written->note == std::optional<std::string>("异步"),
                                   "异步写入的行由同步查询逐列读回");
    }
    ...
    void demonstrateMySqlIntegration()
    {
        ...
        Samples::checklist().check(connection.isConnected() && !connection.serverVersion().empty(), "MySQL 真机建连成功并读到服务端版本号");
        ...
        const ScopedCleanup dropSampleTable([&connection, &tableName] { static_cast<void>(connection.execute("DROP TABLE IF EXISTS " + tableName)); });

        static_cast<void>(connection.execute("DROP TABLE IF EXISTS " + tableName));
        const bool isCreated = connection.execute("CREATE TABLE " + tableName + " (`id` BIGINT PRIMARY KEY, `name` VARCHAR(191) NOT NULL, `note` VARCHAR(191) NULL)") != nullptr;
        ...
        Samples::checklist().check(insertReceipt != nullptr && insertReceipt->affectedRowCount() == 1, "MySQL 写回执给出语句级影响行数（SQLite 之外第二个驱动也覆盖到）");
        ...
        const bool isRolledBack =
                connection.beginTransaction() && connection.execute("INSERT INTO " + tableName + " (`id`, `name`) VALUES (3, '回滚行')") != nullptr && connection.rollback();
        ...
        const bool isMySqlCommitEffective = connection.commit() && scalarInteger(connection, "SELECT COUNT(*) FROM " + tableName) == std::optional<std::int64_t>(3);

        // 负向：重复主键必须给出「中文动作说明 + 服务端原文 + 错误码」而不是崩掉
        const std::unique_ptr<Database::DatabaseResult> duplicateReceipt = connection.execute("INSERT INTO " + tableName + " (`id`, `name`) VALUES (1, '重复主键')");
    ...
    void demonstrateRedisIntegration()
    {
        ...
        const ScopedCleanup removeSampleKeys(
                [&connection, &createdKeys, configuredKeyspace]
                {
        ...
        Samples::checklist().check(isValueRoundTripOkay && isMissingKeyEmpty, "Redis 按参数数组存取：内嵌 '\\0' 的载荷逐字节回读，缺失的键给空结果集而不是空串");

        // 管道：一次 flush 收齐全部回复，顺序与登记顺序严格一致
        const bool isPipelined = connection.pipelineCommand("SET " + secondKey + " alpha") && connection.pipelineCommand("GET " + secondKey) && connection.pipelineCommand("GET");
        ...
        if (replies.size() == 3 && replies[0] != nullptr && replies[1] != nullptr && replies[2] != nullptr)
        {
            isPipelineOrdered = cellAs<std::string>(*replies[0], std::size_t{0}) == std::optional<std::string>("OK") &&
                                cellAs<std::string>(*replies[1], std::size_t{0}) == std::optional<std::string>("alpha") && !replies[2]->lastError().empty();
        }
        Samples::checklist().check(isPipelined && isPipelineOrdered, "管道一次 flush 按登记顺序给出三条回复，其中出错那条的原因留在它自己的结果上");

        // 负向：服务端 error 回复以 nullptr + 中文说明暴露（含服务端原文），连接本身仍可用
        const bool isCommandErrorReported =
                connection.execute("GET") == nullptr && connection.lastError().find("Redis 服务器返回错误") != std::string::npos && containsChinese(connection.lastError());
        Samples::checklist().check(isCommandErrorReported && connection.executeCommand({"PING"}) != nullptr, "Redis 命令报错给出中文前缀，且失败不会把连接本身弄坏");
        ...
        if (connection.selectDatabase(0))
        {
            ...
            {
                isKeyspaceSwitched = isHiddenInDefaultKeyspace && readIntegerReply(connection, {"EXISTS", probeKey}) == std::optional<std::int64_t>(1);
            }
        }
...
int main()
{
    ...
    runGatedStep("MySQL 真机", isMySqlConfigured(), describeMySqlSkipReason(), [] { demonstrateMySqlIntegration(); });
    runGatedStep("Redis 真机", isRedisConfigured(), describeRedisSkipReason(), [] { demonstrateRedisIntegration(); });
