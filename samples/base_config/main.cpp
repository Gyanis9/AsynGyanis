...
namespace
{
    ...
    void demonstrateLoading(const std::filesystem::path &root)
    {
        ...
        writeTextFile(directory / "app.yaml", "server:\n"
                                              "  port: 8080\n"
                                              "  host: 0.0.0.0\n"
                                              "  workers: 4\n"
                                              "feature:\n"
        ...
        Samples::checklist().check(manager.getBool("feature.enable_tls", false) && manager.getDouble("feature.sample_rate", 0.0) == 0.25, "getBool 与 getDouble 按类型取值");

        // 目录式加载：递归子目录，把目录里所有受支持的文件一次读进来
        const Base::ConfigLoadResult fromDirectory = manager.loadFromDirectory(directory, true);
        Samples::checklist().check(fromDirectory.success && fromDirectory.loadedFiles.size() >= 3, "loadFromDirectory 递归读到了子目录里的配置");
        ...
        Samples::checklist().check(reloaded.success && reloaded.loadedFiles.size() == 3 && manager.getInt("server.workers", 0) == 8,
        ...
        Samples::checklist().check(collapsed.success && manager.getInt("server", 0) == 9090 && !manager.has("server.port") && !manager.has("server.host") &&
    ...
    void demonstrateValueAccess(const std::filesystem::path &root)
    {
        ...
        writeTextFile(directory / "typed.yaml", "limits:\n"
        ...
        Samples::checklist().check(manager.get("limits.absent", std::string{"fallback"}) == "fallback", "带默认值的 get 在缺键时回落默认值");
        ...
        Samples::checklist().check(loadedKeys == std::vector<std::string>{"limits.label", "limits.maximum_body", "limits.ratio"}, "keys() 列出已加载的全部键，且按字典序");
        ...
        Samples::checklist().check(missing.size() == 1 && missing.front() == "limits.nope", "validateRequired 只列出缺失的那几个键");
    }
    ...
    void demonstrateSchemaValidation(const std::filesystem::path &root)
    {
        ...
        writeTextFile(directory / "schema-good.yaml", "server:\n  port: 8080\n  host: 0.0.0.0\n  workers: 4\n");
        manager.clear();
        ...
        Samples::checklist().check(!failed.valid && failed.errors.size() >= 3, "缺键、类型不符、越界三类问题都被 schema 校验挑出来");
    ...
    void demonstrateFailurePaths(const std::filesystem::path &root)
    {
        ...
        Samples::checklist().check(!missingFile.success && !missingFile.failedFiles.empty() && !missingFile.errors.empty(), "加载不存在的文件时如实报告失败与原因");
        ...
        Samples::checklist().check(!broken.success && manager.keys().empty(), "语法坏掉的 YAML 被拒，不会装进半份配置");
    }
    ...
    void demonstrateInMemoryOverride(const std::filesystem::path &root)
    {
        ...
        Samples::checklist().check(manager.setValue("cache.ttl", Base::ConfigValue(5)) && manager.getInt("cache.ttl", 0) == 5, "setValue 立即生效于内存快照");
        ...
        Samples::checklist().check(persistedText.find("ttl: 5") == std::string::npos, "setValue 不写回任何文件（磁盘上仍是文件里的取值）");

        // 段名被改写成单个值时，段里那些旧键必须一起让位：否则 getSection 只看得到旧键，
        // 刚设进去的那个值整段读不回来，而 has()/get() 又都报它存在
        Samples::checklist().check(manager.setValue("cache", Base::ConfigValue(1)) && manager.getInt("cache", 0) == 1 && !manager.has("cache.ttl") &&
                                           manager.getSection("cache").empty(),
        ...
        Samples::checklist().check(manager.reload().success && manager.getInt("cache.ttl", 0) == 60 && !manager.has("cache"), "reload 以文件为准，内存里的覆盖随之失效");
    }
    ...
    void demonstrateHotReload(const std::filesystem::path &root)
    {
        ...
        const bool isReloaded = Samples::waitUntil(
                [&manager, &isCallbackFired] { return isCallbackFired.load(std::memory_order_acquire) && manager.getString("runtime.flag") == "on"; }, std::chrono::seconds{8});
        ...
        Samples::checklist().check(isReloadCompletelySuccessful.load(std::memory_order_acquire), "热重载那一轮把目录里的每个配置文件都读进来了（没有部分失败）");
...
int main()
{
    ...
    const auto directory = std::filesystem::temp_directory_path() / ("asyn-sample-base-config-" + std::to_string(Platform::ProcessInfo::currentProcessId()));
