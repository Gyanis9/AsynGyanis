 ...
 *          sql_mode 下是**字符串字面量**，用它引用列名会静默改变语义）、'?' 占位符、分页走
 *          "LIMIT ? OFFSET ?" 且值以绑定参数送出（只给 offset 时补无符号 64 位上界作为「不限行数」）、
 *          事务语句 "START TRANSACTION"、类型名按位宽与符号分家、元数据查 information_schema
 *          并用 DATABASE() 限定当前库、参数上限 65535。
 *
...
namespace AsynGyanis::Database
{
     ...
     *
     * @details 只覆写引擎知识，查询树渲染与参数收集一律继承基类：反引号引用、dialectName() 返回
     *          "MySQL"、分页 limit/offset 都占绑定参数（只给 offset 时补无符号 64 位上界常量）、
     *          占位符 "?"、事务语句 "START TRANSACTION"、参数上限 65535、DDL 支撑见 columnTypeName()。
     *
    ...
    public:
         ...
         * @details 重写 SqlDialect::columnTypeName()：整数按位宽与符号分家，一律选与 C++ 类型
         *          位宽对齐的成员——Int64→"BIGINT"、UInt64→"BIGINT UNSIGNED"、Double→"DOUBLE"、
         *          Text→"TEXT"（上限 65535 字节，按列字符集编码）、Blob→"LONGBLOB"（上限 4 GiB）。
         *          Bool→"TINYINT(1)"：MySQL 没有布尔类型，TINYINT(1) 是官方保留的「是否型」写法
