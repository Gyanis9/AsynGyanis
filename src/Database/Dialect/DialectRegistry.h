 ...
 *
 * @details 上层（Queryable 执行器）只持有 DatabaseType，需要一个把类型映射到 SqlDialect 实例的
 *          统一入口。注册表用函数内静态共享指针持有无状态方言实例，多次取用得到同一个对象
 *          （magic static，线程安全且无需加锁）。
 *          尚未提供方言的类型（如 Oracle）抛中文异常而不是返回空指针：返回空会让每个调用点都必须
 *          写判空分支，漏写一处就是解引用崩溃；不想捕获异常的调用方可用 supports() 预判。
 */
#pragma once
