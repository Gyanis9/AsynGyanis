...
namespace AsynGyanis::Net
{
    ...
    //
    // 尚未接入会话：本片只提供协议机制（帧头与各类型负载的编解码、增量帧解码器），谁调用编码器、
    // 谁把解码器接到连接读取循环上都还没做，因此引入本目录不会改变任何既有会话的行为。
    // 帧层只判**单帧**自身的合法性（长度、标志、padding、优先级字段），不维护流状态、不做
    // 流控记账、不拼头块——那些要看跨帧状态，属于会话层与 HPACK 层。
    // ============================================================================
