...
namespace AsynGyanis::Net
{
    namespace
    {
         ...
         */
        std::array<std::uint8_t, kQuicInitializationVectorByteLength> buildInitializationVector(const QuicPacketKeys &keys, const std::uint64_t packetNumber) noexcept
        {
            ...
            for (std::size_t byteIndex = 0; byteIndex < baseInitializationVector.size(); ++byteIndex)
            {
                ...
                const std::uint8_t packetNumberByte = shiftBitCount >= 64 ? 0 : static_cast<std::uint8_t>((packetNumber >> shiftBitCount) & 0xFFULL);
         ...
         */
        bool runAead(const QuicPacketKeys &keys, const std::uint64_t packetNumber, const std::span<const std::uint8_t> additionalData, const std::span<const std::uint8_t> input,
                     const std::span<std::uint8_t> destination, std::uint8_t *const tag, const bool isEncryption)
        {
            const EVP_CIPHER *const cipher = cipherFor(keys.cipherSuite);
            if (cipher == nullptr)
            {
                throw Base::InvalidArgumentException(std::format("QUIC 包保护失败：套件取值 {} 不是已定义的密码套件：请检查密钥组是怎么填的", static_cast<int>(keys.cipherSuite)));
            }
            if (destination.size() != input.size())
            {
                throw Base::InvalidArgumentException(std::format("QUIC 包保护失败：输出缓冲 {} 字节装不下 {} 字节结果：两者长度必须相同", destination.size(), input.size()));
            }
            ...
            {
                failedStage = "设定密码与方向";
            } else if (EVP_CipherInit_ex(context, nullptr, nullptr, encryptionKey.data(), initializationVector.data(), isEncryption ? 1 : 0) != 1)
            {
                failedStage = "交付密钥与 IV";
            } else if (!additionalData.empty() && EVP_CipherUpdate(context, nullptr, &writtenLength, additionalData.data(), static_cast<int>(additionalData.size())) != 1)
            {
                failedStage = "喂入附加认证数据";
            } else if (!input.empty() && EVP_CipherUpdate(context, destination.data(), &writtenLength, input.data(), static_cast<int>(input.size())) != 1)
            {
                failedStage = isEncryption ? "加密载荷" : "解密载荷";
            } else if (!isEncryption && EVP_CIPHER_CTX_ctrl(context, EVP_CTRL_AEAD_SET_TAG, static_cast<int>(kQuicAuthenticationTagByteLength), tag) != 1)
            {
            ...
            {
                failedStage = "收尾加密";
            } else if (isEncryption && EVP_CIPHER_CTX_ctrl(context, EVP_CTRL_AEAD_GET_TAG, static_cast<int>(kQuicAuthenticationTagByteLength), tag) != 1)
            {
                failedStage = "取出认证标签";
            } else if (!isEncryption && EVP_CipherFinal_ex(context, trailer.data(), &writtenLength) != 1)
            {
            ...
            if (failedStage != nullptr)
            {
                ...
                throw Base::Exception(std::format("QUIC 包保护失败：{}这一步被拒（套件 {}，密钥 {} 字节，IV {} 字节，{}）：OpenSSL 报 {}", failedStage,
                                                  quicCipherSuiteName(keys.cipherSuite), encryptionKey.size(), initializationVector.size(), isEncryption ? "加密" : "解密",
                                                  detail));
            }
            return true;
        }
    ...
    void appendQuicProtectedPayload(std::string &output, const QuicPacketKeys &keys, const std::uint64_t packetNumber, const std::span<const std::uint8_t> additionalData,
                                    const std::span<const std::uint8_t> plaintext)
    {
        ...
        if (packetNumber > kQuicMaximumIntegerValue)
        {
            throw Base::InvalidArgumentException(
                    std::format("包号 {} 超过 QUIC 的上限 {}（RFC 9000 §12.3 的 2^62-1）：请先停止发送，不要用它加密", packetNumber, kQuicMaximumIntegerValue));
        }
        ...
        runAead(keys, packetNumber, additionalData, plaintext, {tail, plaintext.size()}, tail + plaintext.size(), true);
    }

    std::expected<std::size_t, QuicDecodeError> openQuicProtectedPayload(const std::span<std::uint8_t> plaintextOutput, const QuicPacketKeys &keys,
                                                                         const std::uint64_t packetNumber, const std::span<const std::uint8_t> additionalData,
                                                                         const std::span<const std::uint8_t> protectedPayload)
    {
        ...
        {
            throw Base::InvalidArgumentException(std::format("QUIC 解包：明文缓冲是 {} 字节，密文却要产出 {} 字节：请按「密文长度减 {}」准备输出缓冲", plaintextOutput.size(),
                                                             plaintextLength, kQuicAuthenticationTagByteLength));
        }
        ...
        std::copy(protectedPayload.begin() + static_cast<std::ptrdiff_t>(plaintextLength), protectedPayload.end(), tagBuffer.begin());
        if (!runAead(keys, packetNumber, additionalData, ciphertext, plaintextOutput, tagBuffer.data(), false))
        {
            return std::unexpected(QuicDecodeError{QuicDecodeErrorKind::AuthenticationFailed,
                                                   std::format("QUIC 解包：包号 {} 的认证标签校验未通过（套件 {}）：按 RFC 9001 §4.1.4 整包丢弃，不要回错误码", packetNumber,
