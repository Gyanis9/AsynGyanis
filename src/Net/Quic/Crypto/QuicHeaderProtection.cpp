...
namespace AsynGyanis::Net
{
    namespace
    {
         ...
         */
        std::array<std::uint8_t, kQuicHeaderProtectionSampleByteLength> encryptWithElectronicCodebook(const std::span<const std::uint8_t> key,
                                                                                                      const std::span<const std::uint8_t> sample)
        {
            ...
            const bool isSucceeded = EVP_CipherInit_ex(context, cipher, nullptr, key.data(), nullptr, 1) == 1 && EVP_CIPHER_CTX_set_padding(context, 0) == 1 &&
                                     EVP_CipherUpdate(context, output.data(), &outputLength, sample.data(), static_cast<int>(sample.size())) == 1 &&
         ...
         * @throws Base::Exception 运行期故障：上下文建不起来或加密失败
         */
        std::array<std::uint8_t, kQuicHeaderProtectionMaskByteLength> encryptZerosWithChaCha20(const std::span<const std::uint8_t> key, const std::span<const std::uint8_t> sample)
        {
            EVP_CIPHER_CTX *const context = acquireQuicCipherContext();
            if (context == nullptr)
            {
            ...
            const bool isSucceeded = EVP_CipherInit_ex(context, EVP_chacha20(), nullptr, key.data(), sample.data(), 1) == 1 &&
                                     EVP_CipherUpdate(context, mask.data(), &outputLength, zeros.data(), static_cast<int>(zeros.size())) == 1 &&
         ...
         */
        std::expected<std::size_t, QuicDecodeError> packetNumberByteCountOf(const std::uint8_t firstByte, const std::span<const std::uint8_t> packet,
                                                                            const std::size_t packetNumberOffset, const std::string_view operation)
        {
            ...
            if (packetNumberOffset > packet.size() || packet.size() - packetNumberOffset < packetNumberByteCount)
            {
                return std::unexpected(QuicDecodeError{QuicDecodeErrorKind::Truncated,
                                                       std::format("QUIC {}：包号字段需要 {} 字节，偏移 {} 之后只剩 {} 字节，本包只能整包丢弃", operation, packetNumberByteCount,
                                                                   packetNumberOffset, packet.size() > packetNumberOffset ? packet.size() - packetNumberOffset : 0)});
            }
    ...
    std::expected<std::span<const std::uint8_t>, QuicDecodeError> extractQuicHeaderProtectionSample(const std::span<const std::uint8_t> packet, const QuicPacketHeader &header)
    {
    ...
    QuicHeaderProtectionMask generateQuicHeaderProtectionMask(const QuicPacketKeys &keys, const std::span<const std::uint8_t> sample)
    {
        ...
        {
            throw Base::InvalidArgumentException(std::format("套件 {} 的头部保护密钥长度为 0：请先给出有效的密码套件", quicCipherSuiteName(keys.cipherSuite)));
        }
    ...
    std::expected<std::uint8_t, QuicDecodeError> removeQuicHeaderProtection(std::span<std::uint8_t> packet, const QuicPacketHeader &header, const QuicHeaderProtectionMask &mask)
    {
        if (packet.empty())
        {
        ...
        const auto         packetNumberByteCount = packetNumberByteCountOf(unmaskedFirstByte, packet, header.packetNumberOffset, "去头部保护");
        ...
        {
            packet[header.packetNumberOffset + byteIndex] = static_cast<std::uint8_t>(packet[header.packetNumberOffset + byteIndex] ^ mask.bytes[1 + byteIndex]);
        }
        packet[0] = unmaskedFirstByte;
        return unmaskedFirstByte;
    }

    std::expected<std::uint8_t, QuicDecodeError> applyQuicHeaderProtection(std::span<std::uint8_t> packet, const QuicPacketHeader &header, const QuicHeaderProtectionMask &mask)
    {
        if (packet.empty())
        {
        ...
        const auto packetNumberByteCount = packetNumberByteCountOf(header.firstByte, packet, header.packetNumberOffset, "加头部保护");
        if (!packetNumberByteCount.has_value())
        {
            return std::unexpected(packetNumberByteCount.error());
        }
        ...
        const std::uint8_t protectedFirstByte = static_cast<std::uint8_t>(header.firstByte ^ (mask.bytes[0] & maskedBitOf(header.isLongHeader)));
