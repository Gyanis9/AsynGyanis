...
namespace AsynGyanis::Net
{
     ...
     *       不会按可复用连接继续服务。
     * @see HttpRequest::bodyStream(), Router::postStreaming()
     */
    ...
    public:
        ...
        /**
         * @brief 按连接装配：绑定解析器与泵（会话在每次派发前调用）
         * @param parser 本连接的解析器，正文缓冲的唯一真源
         * @param pump 从连接读一批字节并喂给解析器
         */
        void attach(HttpParser &parser, Pump pump);
    ...
    private:
        HttpParser *m_parser{nullptr};        ///< 正文缓冲的解析器（非拥有）
        Pump        m_pump;                   ///< 向连接要字节的泵
        std::string_view m_chunk;             ///< 当前段视图（解析器缓冲或请求对象的内部视图）
