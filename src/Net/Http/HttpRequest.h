...
namespace AsynGyanis::Net
{
    ...
    private:
        ...
        /**
         * @brief URL 百分号解码
         * @details 宽松解码，永不报错："%XX"（XX 为两位十六进制，大小写均可）→ 对应字节，
         *          '+' → 空格（表单编码约定），其余字符原样保留；非法或残缺的序列（"%"、"%4"、
         *          "%%"、"%4G"）连同百分号按原文保留，不吞字符也不报错。越界判据是「剩余长度不小于
         *          一个完整转义序列」而非「下标加二仍小于总长」，因此以 "?q=%41" 收尾时照常解码。
         *          不报错是因为查询串来自外部输入，解码失败不足以否定整个请求。
         * @param source 待解码文本
         * @return 解码后的文本
         */
