...
namespace AsynGyanis::Net
{
     ...
     *
     * @details 会话把承载层交出的 DATA 片段追加进来，处理器经 HttpRequestBody 按批次取走，
     *          于是 h1 那套「拉一次读一次」的语义在 h2/h3 上原样成立。与 h1 的关键差别是背压的
     *          落地方式：h2/h3 各有自己的流控窗口，本类因此在**字节被消费**时才通过消费回调让
     *          会话归还接收窗口——处理器不拉，窗口就不还，对端发到窗口用尽即停。这是真正的背压，
     *          不是「先全收下来再慢慢处理」。
     *
