...
namespace AsynGyanis::Core
{
     ...
     *
     * @details 定时器本身不再持有任何描述符——定时等待全部登记在所属事件循环的
     *          `TimerQueue`（一个循环一个 timerfd，见其说明）里。因此本对象构造与析构
     *          都不产生系统调用，可以放心地按连接、按请求创建。
     *
     *          到期精度由队列统一的毫秒级节拍决定：多个定时器同刻度到期时按截止时间顺序恢复。
     *
     * @note 定时器必须在事件循环运行期间等待；循环停止后未到期的等待不会自行触发，
     *       与「循环停了就没有事件」一致。
     */
    class Timer
    {
