...
namespace AsynGyanis::Core
{
    ...
    IoWatcher::IoWatcher(EventLoop &loop, const int fileDescriptor, const std::uint32_t interests) :
        m_loop(&loop), m_fileDescriptor(fileDescriptor)
    {
        ...
        // 一次注册、连接存活期间不再动用 epoll_ctl：关注的事件位在构造时就定下来。
        // EPOLLET 与就绪缓存配套——边沿只报一次，缓存保证它不会被丢掉
        if (!m_loop->epoll().addFileDescriptor(m_fileDescriptor, interests | EPOLLET, this))
        {
    ...
    void IoWatcher::handleEvents(const std::uint32_t events) noexcept
    {
        ...
        const std::coroutine_handle<> resumableRead =
            (isReadable && m_readWaiter.handle) ? std::exchange(m_readWaiter.handle, nullptr) : std::coroutine_handle<>();
        const std::coroutine_handle<> resumableWrite =
            (isWritable && m_writeWaiter.handle) ? std::exchange(m_writeWaiter.handle, nullptr) : std::coroutine_handle<>();

        if (isReadable)
        {
            if (resumableRead)
            {
                // 先把结果与「已摘除」写回等待器，再恢复协程；此后再不访问等待器指针
                //（它随协程帧，可能已被销毁）。摘除标记是必需的：否则等待器析构时会回来调本对象
                if (m_readWaiter.awaiter != nullptr)
                {
                    m_readWaiter.awaiter->markReady();
                    m_readWaiter.awaiter->markDetached();
                    m_readWaiter.awaiter = nullptr;
                }
            }
        ...
        if (isWritable)
        {
            if (resumableWrite)
            {
                if (m_writeWaiter.awaiter != nullptr)
                {
                    m_writeWaiter.awaiter->markReady();
                    m_writeWaiter.awaiter->markDetached();
                    m_writeWaiter.awaiter = nullptr;
                }
            }
