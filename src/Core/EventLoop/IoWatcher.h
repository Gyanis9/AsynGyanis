...
namespace AsynGyanis::Core
{
     ...
     *
     * @details 关注位按「谁在等」武装，并采**水平触发 + 按需摘除**：一次就绪上报不再重新武装，
     *          只有当「等待者集合」变化时才改内核掩码（无人等的方向在事件处理里收掉，否则内核会
     *          一轮一轮重复上报同一个就绪，把 epoll_wait 拖成空转）。因此常见的 keep-alive 收发
     *          序列（读到数据 → 处理 → 再等）不再表现为「一次就绪一次 epoll_ctl」。
     *          之所以不能只靠「没人等就别武装」而必须显式收掩码：可写几乎长期为真，它要是留在
     *          内核里没人领，就会持续上报。
     *
     * @note 两个后端用同一套语义：Linux 侧是 epoll 的 mod；Windows 侧完成端口的探针本身就是
     *       「一次完成一次上报」，事件循环会在下一次 wait() 前对已上报且仍在掩码里的方向重新投递
     *       探针（等价于水平触发），掩码清零则不再投递。
     *
     * @note 线程约束：**包括析构在内，本类的所有方法都只在所属事件循环线程上调用**。内部状态
     *       因此不需要任何原子或锁；而析构会连带改事件后端的常驻注册表（`delFileDescriptor`），
     *       那本来就是循环线程的私有结构——从别的线程析构等于并发改它。
     *       需要从外部线程停止某个套接字/服务器时，把动作投递到那个循环（见 `TcpServer::stop()`
     *       的 @warning 与 `EventLoop::scheduler().scheduleRemote()`），不要直接调 close()/析构。
     * @note 析构里用 `scheduleRemote()` 唤醒等待者，理由与线程无关：本对象正在析构，就地恢复会让
     *       协程在析构完成之前回来访问成员（那是释放后使用）。
     * @warning 除上述投递方式外，不要从其它线程调用本类的方法。
