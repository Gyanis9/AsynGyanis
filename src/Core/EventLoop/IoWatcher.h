...
namespace AsynGyanis::Core
{
     ...
     *
     * @details 关注位只在有协程等待期间武装，并借 EPOLLONESHOT 让内核在一次上报后自动解除，
     *          因此构造时注册一次后，连接存活期间可无限次等待。必须按需武装的理由是 Windows 侧
     *          用的是 wepoll，它未实现边沿触发（`ASYN_PLATFORM_WIN32` 下 EPOLLET 被定义为 0），
     *          水平触发下长期为真的关注位会在无人等待时让 epoll_wait 全速空转。
     *
