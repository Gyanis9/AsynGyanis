...
namespace AsynGyanis::Core
{
     ...
     *
     * @note 线程约束：等待与恢复两端都在**所属事件循环线程**上，因此内部状态不需要任何原子或锁。
     *       唯一的例外是销毁：外部线程调用 stop()/close() 时本对象可能随之析构（见析构函数说明），
     *       那条路径只负责唤醒等待者，不读改等待槽之外的任何状态。
     * @warning 除销毁外，不要从其它线程调用本类的方法：等待槽与就绪标记都没有同步保护。
