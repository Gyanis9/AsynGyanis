...
int afdCreateHelperHandle(HANDLE iocpHandle, HANDLE *afdHelperHandleOut)
{
    ...
    if (CreateIoCompletionPort(afdHelperHandle, iocpHandle, 0, 0) == nullptr ||
        !SetFileCompletionNotificationModes(afdHelperHandle,FILE_SKIP_SET_EVENT_ON_HANDLE))
    {
 ...
 * @details 低 28 位是引用计数、高 4 位是销毁标记，等待与唤醒走 C++20 的 atomic::wait/notify （标准库在 Windows 上用 WaitOnAddress 实现），原实现自备的 keyed event 因此整块删掉 协议： - ref()：只在计数上加一，无等待；对已销毁的锁调用是用法错误，由断言兜住； - unref()：减一；恰好减到「只剩销毁标记」时唤醒正在等的销毁者； - unrefAndDestroy()：在同一个原子操作里放下自己那份引用并置上销毁标记，然后等其它引用放完 ——这一步是本类比 shared_ptr 多出来的：销毁者要确认没人还在用这块内存； - 销毁完成后写入毒值，此后任何 ref/unref 都会被断言抓住
 */
class RefLock
{
public:
...
#define CONTAINER_OF(pointer, hostType, memberName) \
  ((hostType*) ((uintptr_t) (pointer) - offsetof(hostType, memberName)))
 ...
 * @details 队列本身一次分配都不用做。本文件的两处用法都吃这个代价——每轮 epoll_wait 之后有成百上千 个 socket 要在这几张表之间挪动。改用 std::list 会让每个节点多一次分配，还得把迭代器塞回 节点才能保住同样的摘除代价，得不偿失
 */
struct Queue
{
...
static PollGroup *pollGroupNew(PortState *portState)
{
    ...
    if (afdCreateHelperHandle(iocpHandle, &pollGroup->m_afdHelperHandle) <
        0)
    {
        delete pollGroup;
        return nullptr;
...
PollGroup *pollGroupAcquire(PortState *portState)
{
    ...
    PollGroup *pollGroup      =
            !queueIsEmpty(pollGroupQueue)
                ? CONTAINER_OF(
                        queueLast(pollGroupQueue), PollGroup, m_queueNode)
                : nullptr;

    if (pollGroup == nullptr ||
        pollGroup->m_groupSize >= kMaximumPollGroupSize)
...
static int portPoll(PortState *portState, struct epoll_event *epollEvents, OVERLAPPED_ENTRY *iocpEvents, DWORD maxevents, DWORD timeout)
{
    ...
    BOOL r = GetQueuedCompletionStatusEx(portState->m_iocpHandle,
                                         iocpEvents,
                                         maxevents,
                                         &completionCount,
                                         timeout,
...
{
    if (treeAdd(&portState->m_sockTree,
...
void portAddDeletedSocket(PortState *portState, SockState *sockState)
{
    if (queueIsEnqueued(sockStateToQueueNode(sockState)))
        return;
    queueAppend(&portState->m_sockDeletedQueue,
...
static int sockCancelPoll(SockState *sockState)
{
    ...
    if (afdCancelPoll(pollGroupGetAfdHelperHandle(sockState->m_pollGroup),
                      &sockState->m_ioStatusBlock) < 0)
        return -1;
...
int sockUpdate(PortState *portState, SockState *sockState)
{
    ...
    if ((sockState->m_pollStatus == SockPollStatus::Pending) &&
        (sockState->m_userEvents & kKnownEpollEvents &
         ~sockState->m_pendingEvents) == 0)
    {
    ...
    } else if (sockState->m_pollStatus == SockPollStatus::Idle)
    {
        ...
        sockState->m_pollInfo.Handles[0].Events =
                sockEpollEventsToAfdEvents(sockState->m_userEvents);

        if (afdPoll(pollGroupGetAfdHelperHandle(sockState->m_pollGroup),
                    &sockState->m_pollInfo,
                    &sockState->m_ioStatusBlock) < 0)
        {
            switch (GetLastError())
            {
...
void tsTreeNodeInit(TsTreeNode *node)
{
    ...
    node->m_reflock.ensureInitialized();
}
