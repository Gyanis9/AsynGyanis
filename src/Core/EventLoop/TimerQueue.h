...
namespace AsynGyanis::Core
{
     ...
     *          socketpair），全部定时等待按截止时间排在同一个最小堆里。相比「每个定时器自己占
     *          一个描述符」，N 个定时器的代价从 2N 个描述符（外加 N 次 loopback 建对）降到
     *          1 个描述符加 O(log N) 的堆操作——连接级定时器因此可以随便加。
     *
     *          队列由一个驱动协程推进：按堆顶截止时间武装描述符，等它可读（复用 IoWatcher 的
     *          按需武装，空队列时不武装，因此空闲循环不会被定时器唤醒），到期后把到期的等待者
     *          逐个投回调度器。走调度器而不是就地恢复，是为了不在驱动协程里嵌套恢复别的协程。
     *          驱动协程**在第一次登记定时等待时才启动**，因此没用过定时器的循环完全不受影响，
     *          新建的循环也保持「没有任何待办」。
     *
     *          循环停止后未到期的等待者不会自行触发——它们随各自协程帧销毁，与「循环停了就
     *          没有事件」的既有语义一致。
     *
