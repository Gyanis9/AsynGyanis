...
namespace AsynGyanis::Core
{
     ...
     *          形成无效释放并破坏堆。
     *
     *          **锁的代价被压到零**：本池是全引擎最热路径上的一环——每次 `co_await
     *          socket.asyncReceive()` 都会新建一个协程帧，于是「每完成一次 I/O 就分配并
     *          释放一个帧」。若每次都取全局互斥锁，多线程下所有 EventLoop 会在同一把锁上
     *          排队，核数越多越差。因此采用「每线程本地缓存 + 全局池兜底」：
     *          - 稳态（本地缓存非空）下 `allocate()` / `deallocate()` **不取锁、不做原子操作**，
     *            只操作本线程的空闲链表；
     *          - 本地缓存空时一次性从全局池搬一批（`kLocalCacheCapacity` 块）过来，把取锁频率
     *            摊薄到 1/64；
     *          - 线程退出时把缓存里的块全部归还全局池，避免块滞留在已结束的线程上。
     *          跨线程归还依旧安全：块是否属于本池由「内存段 + 段内偏移」判定，
     *          与由哪个线程分配无关；判定路径无锁（段描述写完后才发布段数量）。
     *          代价是某线程缓存里的块在它退出前对其他线程不可见（每个线程最多滞留
     *          kLocalCacheCapacity 块），这是每线程缓存的固有取舍，上限仅 16 KB。
     *
