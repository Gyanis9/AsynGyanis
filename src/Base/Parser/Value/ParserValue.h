...
namespace AsynGyanis::Base
{
    ...
    using ParserValueArray  = std::vector<ParserValue>;                         ///< 文档数组：ParserValue 有序列表
    using ParserValueObject = std::map<std::string, ParserValue, std::less<> >; ///< 文档对象：按键有序的嵌套映射

    // 为 ParserValueArray / ParserValueObject 特化 typeNameOf（必须在 ParserValueType.h 主模板之后）
    template<>
    [[nodiscard]] inline const char *typeNameOf<ParserValueArray>() noexcept
    {
        return "array";
    }

    template<>
    [[nodiscard]] inline const char *typeNameOf<ParserValueObject>() noexcept
    {
        return "object";
    }

    /**
     * @brief 判断类型是否为 ParserValue 底层变体的直接存储类型。
     * @details 取代原先按类型逐个特化 std::true_type 的写法，约束语义不变：
     *          只接受变体的精确替代类型，隐式可转换类型（如 int、const char *）不满足。
     * @tparam T 待判定的类型
     */
    template<typename T>
    concept ParserValueAlternative = std::same_as<T, std::nullptr_t> ||
                                     std::same_as<T, bool> ||
                                     std::same_as<T, std::int64_t> ||
                                     std::same_as<T, double> ||
                                     std::same_as<T, std::string> ||
                                     std::same_as<T, ParserValueArray> ||
                                     std::same_as<T, ParserValueObject>;
    ...
    public:
        ...
        // 变体下标必须与 ParserValueType 枚举值一一对应，type() 依赖该映射
        ...
        static_assert(std::is_same_v<std::variant_alternative_t<5, VariantType>, ParserValueArray>);
        static_assert(std::is_same_v<std::variant_alternative_t<6, VariantType>, ParserValueObject>);

        /**
         * @brief 构造空文档值（Null）。
         */
        ParserValue() noexcept;

        /**
         * @brief 从空指针字面量构造 Null 文档值。
         * @param value 空指针字面量，仅用于选择重载。
         */
        explicit ParserValue(std::nullptr_t value) noexcept;

        /**
         * @brief 从布尔值构造文档值。
         * @param value 布尔值。
         */
        explicit ParserValue(bool value) noexcept;

        /**
         * @brief 从整型值构造文档值。
         * @param value 整型值，内部提升为 int64_t 存储。
         */
        explicit ParserValue(int value) noexcept;

        /**
         * @brief 从 64 位整型构造文档值。
         * @param value 64 位整型值。
         */
        explicit ParserValue(int64_t value) noexcept;

        /**
         * @brief 从双精度浮点值构造文档值。
         * @param value 浮点值。
         */
        explicit ParserValue(double value) noexcept;

        /**
         * @brief 从 C 字符串构造文档值，空指针按空字符串处理。
         * @param value C 字符串。
         */
        explicit ParserValue(const char *value);

        /**
         * @brief 从字符串构造文档值。
         * @param value 字符串值。
         */
        explicit ParserValue(std::string value) noexcept;

        /**
         * @brief 从数组对象构造文档值。
         * @param value 数组值。
         */
        explicit ParserValue(ParserValueArray value) noexcept;

        /**
         * @brief 从对象映射构造文档值。
         * @param value 对象值。
         */
        explicit ParserValue(ParserValueObject value) noexcept;
         ...
         */
        ParserValue(const ParserValue &other) = default;
         ...
         * @param other 待移源的文档值。
         */
        ParserValue(ParserValue &&other) noexcept = default;

        /**
         * @brief 拷贝赋值运算符。
         * @details 默认逐成员赋值，自赋值安全。
         * @param other 待拷贝的源文档值。
         * @return ParserValue& 本对象引用。
         */
        ParserValue &operator=(const ParserValue &other) = default;
         ...
         * @return ParserValue& 本对象引用。
         */
        ParserValue &operator=(ParserValue &&other) noexcept = default;
        ...
        /**
         * @brief 获取值的类型
         * @return ParserValueType 当前值的类型枚举。
         */
        [[nodiscard]] ParserValueType type() const noexcept;
        ...
        /**
         * @brief 将值转换为数组类型。
         * @return const ParserValueArray& 数组引用。
         * @throws ValueAccessError 类型不匹配时抛出。
         */
        [[nodiscard]] const ParserValueArray &asArray() const;

        /**
         * @brief 将值转换为对象类型。
         * @return const ParserValueObject& 对象引用。
         * @throws ValueAccessError 类型不匹配时抛出。
         */
        [[nodiscard]] const ParserValueObject &asObject() const;
        ...
        [[nodiscard]] std::optional<std::decay_t<T> > get() const noexcept
        {
            ...
            } else if constexpr (std::is_floating_point_v<Target>)
            {
                ...
                return std::nullopt;
            } else if constexpr (ParserValueAlternative<Target>)
            {
        ...
        /**
         * @brief 安全获取数组值。
         * @return std::optional<ParserValueArray> 类型匹配时返回值，否则返回空。
         */
        [[nodiscard]] std::optional<ParserValueArray> getArray() const noexcept;

        /**
         * @brief 安全获取对象值。
         * @return std::optional<ParserValueObject> 类型匹配时返回值，否则返回空。
         */
        [[nodiscard]] std::optional<ParserValueObject> getObject() const noexcept;
         ...
         * @param key 配置键。
         * @return const ParserValue& 对应文档值引用。
         * @throws ValueAccessError 当前值不是对象时抛出。
         * @throws ValueAccessError 键不存在时抛出。
         */
        const ParserValue &operator[](std::string_view key) const;

        /**
         * @brief 安全通过键访问对象成员。
         * @param key 配置键。
         * @return std::optional<std::reference_wrapper<const ParserValue>> 键存在时返回值引用。
         */
        [[nodiscard]] std::optional<std::reference_wrapper<const ParserValue> > get(std::string_view key) const noexcept;

        /**
         * @brief 通过键安全访问子文档值并转换为指定类型
         * @tparam T 目标类型 (bool, int64_t, double, std::string, ParserValueArray, ParserValueObject)
         * @param key 配置键
         ...
         * @param index 数组下标。
         * @return const ParserValue& 对应元素引用。
         * @throws ValueAccessError 当前值不是数组时抛出。
         * @throws ValueAccessError 下标越界时抛出。
         */
        const ParserValue &operator[](size_t index) const;
