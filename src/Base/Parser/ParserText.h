...
namespace AsynGyanis::Base
{
     ...
     *          所有失败路径抛出 ParserError 并带上调用方传入的位置。
     */
    class ParserText
    {
    public:
         ...
         * @param position 出错时上报的位置基准
         * @return char 转义代表的字符
         * @throws ParserError 出现未知转义字符
         */
        static char decodeSimpleEscape(std::string_view text, std::size_t &index, const ParserPosition &position);
         ...
         * @throws ParserError 转义非法或代理对不完整
         */
        static std::string decodeQuotedBody(std::string_view body, const ParserPosition &position);
    };
} // namespace AsynGyanis::Base
