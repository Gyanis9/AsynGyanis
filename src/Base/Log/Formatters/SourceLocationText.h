...
namespace AsynGyanis::Base
{
     ...
     *
     * @details 用 `std::format_to_n` 只往给定缓冲写、不分配，并返回「装下整段所需长度」，
     *          据此判断是否被截断：装得下就以 string_view 指向缓冲内的文本，装不下才返回
     *          空视图，由调用方自行回退到原先的分配路径。两条路径共用同一套格式说明
     *          `"{}:{}"`，因此文本内容与长度完全一致，只差内存来源。
     *
     *          空视图当且仅当未命中：文本至少含一个 ':' 与一个行号数字，长度恒不小于 2，
     *          装得下时不可能为空。`noexcept` 成立的原因是格式串是编译期字面量（不在运行期
     *          解析，不会抛 format_error），且写入固定缓冲不涉及任何分配。
