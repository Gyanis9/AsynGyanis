...
namespace AsynGyanis::Base
{
    namespace
    {
         ...
         */
        template<typename ValueType>
        [[nodiscard]] ValueType optionalFieldWithDiagnosis(const ConfigValue &configuration, const std::string_view key, ValueType defaultValue,
                                                           const std::string_view ownerDescription)
        {
            ...
            if (!parsed.has_value())
            {
                std::cerr << "LoggerConfig：" << ownerDescription << " 的 " << key << " 类型是 " << typeName((*iterator).type()) << "，要求 " << configTypeNameOf<ValueType>()
         ...
         */
        [[nodiscard]] std::optional<std::string> requiredStringField(const ConfigValue &configuration, const std::string_view key, const std::string_view ownerDescription)
        {
            const auto iterator = configuration.find(key);
            if (iterator == configuration.end())
            {
            ...
            if (!value.has_value())
            {
                std::cerr << "LoggerConfig：" << ownerDescription << " 的 '" << key << "' 类型是 " << typeName((*iterator).type()) << "，要求 " << configTypeNameOf<std::string>()
                          << "，已跳过该 sink" << '\n';
            }
            return value;
        }
        ...
        {
            const std::filesystem::path basePath     = baseDirectory.empty() ? AsynGyanis::Platform::ProcessInfo::applicationDirectory() : baseDirectory;
    ...
    void LoggerConfigLoader::loadFromConfig(const std::string &configurationPrefix, const std::filesystem::path &baseDirectory)
    {
        ...
        {
            std::cerr << "LoggerConfig：" << configurationPrefix << " 段不是对象（实际是 " << typeName(loggingSection.type()) << "），本次不改动任何日志器" << '\n';
            return;
        }
        ...
        if (const auto globalLevelIterator = loggingSection.find("global_level"); globalLevelIterator != loggingSection.end())
        {
            ...
            {
                std::cerr << "LoggerConfig：" << configurationPrefix << ".global_level 类型是 " << typeName(globalLevelIterator->type()) << "，要求 "
                          << configTypeNameOf<std::string>() << "，已按 INFO 处理" << '\n';
            }
        }
        ...
        {
            std::cerr << "LoggerConfig：" << configurationPrefix << ".loggers 不是对象（实际是 " << typeName(loggersIterator->type())
                      << "，每个日志器要写成 '名字:' 加它的字段），本次不改动任何日志器" << '\n';
            return;
        }
        ...
        for (const auto &[loggerName, loggerConfiguration]: loggers)
        {
            ...
            {
                std::cerr << "LoggerConfig：日志器 '" << loggerName << "' 的配置不是对象（实际是 " << typeName(loggerConfiguration.type())
                          << "，它的字段要写成 'level:' 与 'sinks:' 那样的键值对），已跳过该日志器" << '\n';
                continue;
            }
    ...
    std::unique_ptr<LogSink> LoggerConfigLoader::createSinkFromConfig(const ConfigValue &sinkConfiguration, const std::filesystem::path &baseDirectory)
    {
        if (!sinkConfiguration.is_object())
        {
            ...
            std::cerr << "LoggerConfig：sink 配置不是对象（实际是 " << typeName(sinkConfiguration.type()) << "），已跳过该 sink；每条 sink 要写成 '- type: ...' 加它的字段" << '\n';
            return nullptr;
        }
        ...
        } else if (type == "rolling_file")
        {
            ...
            const std::filesystem::path logDirectory =
                    resolveLogPath(optionalFieldWithDiagnosis<std::string>(sinkConfiguration, "directory", std::string{"logs"}, "rolling_file sink"), baseDirectory);
            const std::string policyName = optionalFieldWithDiagnosis<std::string>(sinkConfiguration, "policy", std::string{"size"}, "rolling_file sink");
            ...
            else
            {
                ...
                std::cerr << "LoggerConfig：rolling_file sink 的 policy='" << policyName << "' 非法（只支持 size / daily / hourly），已按 size 处理" << '\n';
                policy = RollingPolicy::Size;
            }

            const int64_t configuredMaximumSizeMb = optionalFieldWithDiagnosis(sinkConfiguration, "max_size_mb", int64_t{10}, "rolling_file sink");
            ...
            const int64_t     clampedMaximumSizeMb = std::clamp(configuredMaximumSizeMb, int64_t{1}, maximumSizeMbLimit);
            if (clampedMaximumSizeMb != configuredMaximumSizeMb)
            {
                std::cerr << "LoggerConfig：rolling sink 的 max_size_mb=" << configuredMaximumSizeMb << " 非法（要求 1~" << maximumSizeMbLimit << "），已钳制为 "
                          << clampedMaximumSizeMb << "；单个日志文件按钳制后的大小滚动" << '\n';
            }
            ...
            // 0 是合法值：不保留任何备份
            const int64_t     configuredMaximumBackupCount = optionalFieldWithDiagnosis(sinkConfiguration, "max_backup", int64_t{10}, "rolling_file sink");
            ...
            {
                std::cerr << "LoggerConfig：rolling sink 的 max_backup=" << configuredMaximumBackupCount << " 非法（要求 0~" << maximumBackupLimit << "），已钳制为 "
                          << clampedMaximumBackupCount << "；请检查该键是否多打了一位数字，日志保留份数按钳制后的值生效" << '\n';
            }
            ...
            {
                std::cerr << "LoggerConfig：rolling_file sink 的 base_filename='" << *baseOptional << "' 带目录段，只取其中的文件名 '"
                          << AsynGyanis::Platform::FileSystem::utf8FromPath(basePath.filename()) << "'；要把日志放进子目录请写在 directory 字段里" << '\n';
            }
            sink = std::make_unique<RollingFileSink>(basePath, logDirectory, policy, maximumSizeBytes, maximumBackupCount);
        } else if (type == "async")
        {
            ...
            const int64_t configuredQueueSize = optionalFieldWithDiagnosis(sinkConfiguration, "queue_size", int64_t{1024}, "async sink");
            ...
            {
                std::cerr << "LoggerConfig：async sink 的 queue_size=" << configuredQueueSize << " 非法（要求 " << minimumQueueSizeLimit << "~" << maximumQueueSizeLimit
                          << "），已钳制为 " << clampedQueueSize << "；队列容量按钳制后的值生效" << '\n';
            }
            const std::string overflowPolicyName = optionalFieldWithDiagnosis<std::string>(sinkConfiguration, "overflow_policy", std::string{"block"}, "async sink");
            ...
            {
                std::cerr << "LoggerConfig：async sink 的 overflow_policy='" << overflowPolicyName << "' 非法（只支持 block / drop / drop_oldest），已按 block 处理" << '\n';
            }
        ...
        if (sink && sinkConfiguration.contains("formatter"))
        {
            ...
            const std::string formatterName = optionalFieldWithDiagnosis<std::string>(sinkConfiguration, "formatter", std::string{"default"}, "sink");
            ...
            {
                std::cerr << "LoggerConfig：sink 的 formatter='" << formatterName << "' 非法（只支持 default / color / json），已按 default 处理" << '\n';
                if (consoleSink != nullptr)
                {
                    consoleSink->setColorEnabled(false);
                } else
