...
namespace AsynGyanis::Base
{
    ...
    bool LogThrottle::acquire() noexcept
    {
        ...
        if (now < nextPassAtMilliseconds ||
            !m_nextPassAtMilliseconds.compare_exchange_strong(
                    nextPassAtMilliseconds,
                    now + static_cast<std::uint64_t>(m_interval.count()),
        ...
        // 既不会少一份计数也不会少一条日志
        m_droppedReported.store(m_droppedSincePass.exchange(0U, std::memory_order_relaxed),
                                std::memory_order_relaxed);
        return true;
    }
