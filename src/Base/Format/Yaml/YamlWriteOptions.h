...
namespace AsynGyanis::Base
{
     ...
     * @brief 多行字符串的标量风格策略
     *
     * @details 只影响「含换行符的字符串」如何落地，单行字符串一律走裸标量/引号标量：
     *          - Literal：块标量 `|`（§8.1.1），逐字节保留每一行与换行，唯一无损的选择；
     *          - Folded：块标量 `>`（§8.1.3），散文式折行更省行数，但**折叠本身有损**，
     *            因此仅在文本可无损折叠时启用，否则自动回退为 Literal；
     *          - Quoted：双引号标量（§7.3.2）配 `\n` 转义，单行输出，便于把多行文本
     *            塞进不便换行的位置（如需要与其它工具做逐行 diff 的场景）。
     */
