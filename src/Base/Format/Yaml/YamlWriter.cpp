...
namespace AsynGyanis::Base
{
    namespace
    {
         ...
         * @details 逐条对齐扫描器的裸标量规则（§7.3.3 的 `c-ns-plain-safe`）：
         *          - 首字符不得是指示符，也不得是空白（首尾空白会被裁掉）；
         *          - 不得含控制字符（含换行与制表符）；
         *          - ` #` 是注释引导、`: ` 是键值分隔，都不属于裸标量内容（§6.2、§7.4.2）；
         *          - 引号一律排除：扫描器定位键值分隔时会先做引号配对，孤立的引号会让整行无法识别；
         *          - flow 上下文额外排除 `,` `[` `]` `{` `}` `:` `#`（flow 的分隔符集合，§7.4）。
         * @param text 待判定文本
        ...
        private:
             ...
             *          行会被吃掉缩进（§8.1.1.1），只有显式写出缩进位数才可靠。
             *
             *          chomping 按尾随换行数 N 三态落地（§8.1.1.2）——注意 keep 的「保留全部」里
             *          已经包含正文末行自身的那个换行：
             *          - N == 0：写 `-`（strip），正文行尾写出的那个换行被丢弃，回读恰为 0 个；
             *          - N == 1：不写任何指示符（clip，默认），正文行尾的换行被保留，回读恰为 1 个；
             *          - N >= 2：写 `+`（keep），正文末行的换行之外每个尾随空行再贡献一个换行，
             *            故补 N-1 个空行（正文末行自带的换行已是第 1 个）。
             ...
             * @param value 待判定值
             * @return true 该结构此前已定义过锚点，应写成 `*aN`
             */
