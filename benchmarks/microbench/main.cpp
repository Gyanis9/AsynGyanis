...
int main(int argumentCount, char **argumentValues)
{
    ...
    // 整份快照，键越多越贵。本机（Windows/MSVC Release）连跑三次的读数，据此登记基线：
    //   config-get-int      28.4 / 28.8 / 29.0 ns（256 键的快照里取一个整数）
    //   config-set-value  11 711.9 / 11 732.5 / 11 802.7 ns（同一份快照上覆盖一个已存在的键）
    // 容器（GCC 13 Release）同法读数 30.4~32.1 ns 与 9 840~9 969 ns —— 跨编译器同量级，当旁证。
    // 差在哪：把「定形扫描」单独摘掉再量一次是 9 908 / 9 926 ns（容器），与带着它时同量级，
    // 也就是说这 10~12 µs 全是「复制整份快照」的钱，为正确性加的那次全表扫描不到 1%。反过来这也
