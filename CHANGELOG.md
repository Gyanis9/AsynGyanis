...
- **JSON 版式直接摊进 Sink 的行缓冲**：`JsonFormatter` 原先只实现 `format()`，Sink 走的是基类那条
  「先 `format()` 出一份临时串、再整行 append」的通道，那条临时串本身还要按字符几何增长重分配七八次。
  现在 `JsonFormatter` 覆写 `formatInto()`，用同一份序列化器直接把对象追加进调用方的缓冲——Sink 的
  行缓冲跨行留着容量，稳态下这一段不再为输出串取堆。转义表、键序与非法 UTF-8 的抛点都由库自己那一份
  序列化代码负责，产物与 `dump()` 逐字节相同（用例把这条钉成「自身解析后重新序列化是恒等」的不动点判据，
  并新增「缓冲里已有的内容不被碰」与「抛出时一字未增」两条）。
  实测（Release、MSVC）：微基准 `log-format-json` **1529.4 → 1133.4 ns/行（−26%）**；
  分配读数（`AllocationProbe`、一千行同一条事件）Sink 通道 **约 24 → 18 次、1408 字节**，
  独立 `format()` 出口 **23 → 19 次、1584 字节**。文本版式的同一条事件仍是 1 次 / 112 字节——
  剩下的十七次都在「建 nlohmann DOM」那一段，是下一档的靶子。
  实现上取的是 nlohmann 的 `detail::` 序列化器（`dump()` 没有「追加到既有缓冲」的公开出口），
