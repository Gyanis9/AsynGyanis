...
- **HTTP/2 服务过静态文件后立即归还该文件的映射**：会话的响应对象按连接复用，而正文在发出时
  ...
  Windows 上发布方就地改名或截断该文件会被挡最长一个空闲超时，POSIX 上则是一条 fd 与一段地址
  空间预留按空闲连接计（HTTP/1.1 侧早已在发送完成后解除，两条协议从此一致）。新增一条 h2c
...
- **Windows 的静态正文改为一次读取，不再为每个请求建映射**：映射的卖点是零拷贝，但本平台没有任何零拷贝
  ...
  长度表达而不是报错或补零），200/206 的正文长度与 HEAD 的 `content-length` 收敛成同一个数。
